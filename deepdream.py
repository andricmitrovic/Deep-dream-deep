...
if torch.cuda.is_available():
    ...
    device = torch.device('cpu'); print("GPU isn't available! :(")
...
root_path = "./";
img_path = root_path + "starry_night.jpg";
img_sz = 512

helper = Helper(img_sz=img_sz, resize=True, pres_aspect_ratio=True)
image_ = helper.read_img(img_path);
helper.show_img(image_)

dreamer = DeepDream(image_, layers=["conv4_4"]);
output = dreamer.deepdream(lr=5e-3)
